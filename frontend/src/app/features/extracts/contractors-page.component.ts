...
<div class="page-container fade-in">
  <div class="page-header">
    <div class="page-title">
      ...
      <p>{{ isAr() ? 'إدارة قائمة المقاولين المسجلين في النظام' : 'Manage registered contractors' }}</p>
    </div>
  ...
  <div class="search-wrap tf-card">
    ...
    <input class="search-input" [(ngModel)]="searchTerm" (ngModelChange)="onSearch()"
      [placeholder]="isAr() ? 'ابحث بالاسم أو جزء منه...' : 'Search by name...'" />
  ...
  <div class="cards-grid" *ngIf="!loading()">
    <div class="entity-card tf-card" *ngFor="let c of filtered()">
      <div class="card-header">
        ...
        <div class="card-badge">
          <mat-icon>receipt_long</mat-icon> {{ c._count?.extracts ?? 0 }}
        </div>
      </div>
      <div class="card-body">
        ...
        <div class="detail-row" *ngIf="!c.phone && !c.email">
          <mat-icon>info</mat-icon>
          <span class="text-muted">{{ isAr() ? 'لا توجد بيانات تواصل' : 'No contact info' }}</span>
        </div>
      </div>
      <div class="card-footer">
        ...
        <button mat-icon-button color="primary" (click)="openForm(c)" *ngIf="canWrite()"
    ...
    <div class="empty-state" *ngIf="filtered().length === 0">
      ...
      <p>{{ searchTerm ? (isAr() ? 'لا توجد نتائج للبحث' : 'No search results') : (isAr() ? 'أضف أول مقاول للنظام' : 'Add the first contractor') }}</p>
      <button mat-raised-button color="primary" (click)="openForm(null)" *ngIf="canWrite() && !searchTerm">
        <mat-icon>add</mat-icon> {{ isAr() ? 'إضافة مقاول' : 'Add Contractor' }}
      </button>
    </div>
  </div>
</div>
...
<div class="form-overlay" *ngIf="showForm()">
  ...
  <div class="form-drawer tf-card">
    ...
    <div class="form-body">
      <mat-form-field appearance="outline" class="w100">
        ...
        <input matInput [(ngModel)]="form.name" [placeholder]="isAr() ? 'اسم المقاول بالإنجليزية' : 'Contractor name'">
      </mat-form-field>
      <mat-form-field appearance="outline" class="w100">
        <mat-label>{{ isAr() ? 'الاسم بالعربي' : 'Arabic Name' }}</mat-label>
        <input matInput [(ngModel)]="form.nameAr" placeholder="اسم المقاول بالعربي">
      </mat-form-field>
      <mat-form-field appearance="outline" class="w100">
  ...
  styles: [`
    ...
    .avatar {
      width:48px; height:48px; border-radius:14px; background:linear-gradient(135deg,var(--color-primary),var(--color-primary-light));
      color:#fff; display:flex; align-items:center; justify-content:center; font-size:20px; font-weight:800; flex-shrink:0;
    }
    ...
    .form-drawer{position:relative;z-index:201;width:480px;max-width:calc(100vw - 40px);height:min(90vh,520px);display:flex;flex-direction:column;overflow:hidden;}
...
export class ContractorsPageComponent implements OnInit {
  ...
  openForm(item: any | null) {
    ...
    this.form   = item ? { name: item.name, nameAr: item.nameAr, phone: item.phone, email: item.email } : {};
  ...
  save() {
    ...
    const req$ = this.editId
      ? this.svc.updateContractor(this.editId, this.form)
      : this.svc.createContractor(this.form);
    req$.subscribe({
      next: () => {
