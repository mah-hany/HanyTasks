...
@Component({
  ...
  template: `
    <div class="page-container fade-in">
      ...
      <div class="form-drawer tf-card" *ngIf="showForm()">
  ...
  styles: [`
    ...
    .form-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.4); z-index: 200; }
    .form-drawer {
      position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
      width: 520px; max-width: 95vw; max-height: 90vh; overflow-y: auto; z-index: 201;
      padding: 0;
    }
    .form-header { display: flex; align-items: center; justify-content: space-between; padding: 20px 24px 0; }
    .form-body { padding: 16px 24px; display: flex; flex-direction: column; gap: 12px; }
    .form-footer { display: flex; justify-content: flex-end; gap: 8px; padding: 0 24px 20px; }
    ...
    .inline-input { flex: 1; border: 1px solid var(--border-color); border-radius: 6px; padding: 6px 10px; font-size: 13px; outline: none; background: var(--bg-main); color: var(--text-primary); &:focus { border-color: var(--color-primary-light); } }
  `]
})
