...
router.get('/leaderboard', async (req, res, next) => {
    try {
        ...
        if (period === 'week')
            ...
            from = new Date(now.getFullYear(), now.getMonth(), 1); // month
        // Aggregate points per user
        const pointsAgg = await client_1.default.userPoint.groupBy({
            by: ['userId'],
        ...
        const leaderboard = await Promise.all(pointsAgg.map(async (p, idx) => {
            const user = userMap.get(p.userId);
            const [completed, total, onTime] = await Promise.all([
                client_1.default.task.count({ where: { assignedToId: p.userId, status: 'COMPLETED', completedDate: { gte: from } } }),
                client_1.default.task.count({ where: { assignedToId: p.userId, createdAt: { gte: from } } }),
...
router.get('/me', async (req, res, next) => {
    try {
        ...
        const [totalPoints, monthPoints, entries, completedTotal] = await Promise.all([
            client_1.default.userPoint.aggregate({ where: { userId }, _sum: { points: true } }),
            client_1.default.userPoint.aggregate({ where: { userId, createdAt: { gte: monthStart } }, _sum: { points: true } }),
...
function getLevel(points) {
    if (points < 100)
        return { level: 1, name: 'Beginner', nameAr: 'مبتدئ', nextLevelAt: 100 };
    if (points < 300)
        return { level: 2, name: 'Contributor', nameAr: 'مساهم', nextLevelAt: 300 };
    if (points < 600)
        return { level: 3, name: 'Achiever', nameAr: 'منجز', nextLevelAt: 600 };
    if (points < 1000)
        return { level: 4, name: 'Expert', nameAr: 'خبير', nextLevelAt: 1000 };
    if (points < 2000)
        return { level: 5, name: 'Master', nameAr: 'متمكن', nextLevelAt: 2000 };
