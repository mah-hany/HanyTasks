...
// ── Export Tasks as PDF ──────────────────────────────────────────
router.get('/tasks/pdf', async (req, res, next) => {
    try {
        const { status, priority, from, to, assignedToId } = req.query;
        const where = {};
        if (status)
    ...
    `;
        const pdf = await (0, pdf_service_1.generatePdf)(html, { landscape: false });
...
const pdf_service_1 = require("../../shared/utils/pdf.service");
// ── Export Extracts as PDF ──────────────────────────────────────────
router.get('/extracts/pdf', async (req, res, next) => {
    try {
        const { status, projectId, contractorId, dateFrom, dateTo } = req.query;
        const where = {};
        if (projectId)
...
router.get('/report/employee/:id/pdf', async (req, res, next) => {
    try {
        const userId = +req.params.id;
        const { from, to } = req.query;
        const dateFilter = from || to ? {
